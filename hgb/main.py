...
def main(args):
    ...
    if args.dataset != ['Ecomm','Aminer']:
        ...
        if args.num_hops == 1:
            ...
            save_name = f'./EA_adjs/feat_hop{args.num_hops}'
    ...
    if args.dataset in ['DBLP', 'ACM', 'IMDB','Yelp']:
        data_size = {k: v.size(-1) for k, v in raw_feats.items()}
    elif args.dataset == ['Ecomm','Aminer']:
    ...
    for seed in args.seeds:
        ...
        if args.label_feats:
            ...
            prop_tic = datetime.datetime.now()
            if args.dataset == ['Ecomm','Aminer'] and args.num_label_hops <= args.num_hops and len(extra_metapath) == 0:
                meta_adjs = {k: v for k, v in raw_meta_adjs.items() if k[-1] == '0' and len(k) < max_length}
            else:
                if args.dataset == ['Ecomm','Aminer']:
                    save_name = f'./EA_adjs/label_seed{args.seed}_hop{args.num_label_hops}'
        ...
        for epoch in tqdm(range(args.epoch)):
            ...
            if (args.dataset != ['Ecomm','Aminer'] and loss_val < best_val_loss) or (args.dataset == ['Ecomm','Aminer'] and sum(val_acc) > sum(best_val)):
                best_epoch = epoch
...
def parse_args(args=None):
    parser = argparse.ArgumentParser(description='HGAC')

    parser.add_argument('--seeds', nargs='+', type=int, default=[1],
                        help='the seed used in the training')
    parser.add_argument('--dataset', type=str, default='DBLP',
                        choices=['DBLP', 'ACM', 'IMDB', 'Yelp','Ecomm','Aminer'])
