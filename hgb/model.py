...
from data.utils import make_hodata
...
class Transformer(nn.Module):
    '''
        The transformer-based semantic fusion in HGAC.
    '''
...
class LinearPerMetapath(nn.Module):
    '''
        Linear projection per metapath for atteribute projection in HGAC-LLM.
    '''
...
class HGAC(nn.Module):
    ...
    def __init__(self, dataset, nfeat, hidden, nclass, feat_keys, label_feat_keys, tgt_type,
                 ...
                 residual=False, data_size=None, drop_metapath=0.,num_heads=1,llm_graph_emb = None,):
        super(HGAC, self).__init__()
        self.hgnn_ac = HGAC(in_dim=nfeat, hidden_dim=hidden, dropout=dropout,
                               activation=F.elu, num_heads=num_heads)
        ...
        self.graph_mlp = torch.nn.Sequential(
                            torch.nn.Linear(in_features=1536,
                                            ...
                                            bias=True),
                            torch.nn.ReLU(),
                            torch.nn.Linear(in_features=hidden*2,
                                            out_features=hidden,
                                            bias=True),
        )
        ...
        if self.dataset not in ['IMDB', 'Ecomm', 'Aminer']:
    ...
    def forward(self, batch, feature_dict, label_dict={}, mask=None):
        ...
        for i in range(len(llm_cate_embs)):
            embedding_vector = llm_cate_embs[i].to(self.device)
            self.graph_mlp .to(embedding_vector.device)
            llm_cate_embs.append(self.graph_mlp(embedding_vector))
        norm = np.linalg.norm(llm_cate_embs - fc_after_concat)
        lambda_i = max(1 - 1 / norm, 0)
        # AC
        fc_after_concat = lambda_i * fc_after_concat + (1 - lambda_i) * llm_cate_embs
